...
import { Plus, Search, Edit2, Trash2, Wrench, Clock, CheckCircle, Camera, Zap } from 'lucide-react';
...
const osSchema = z.object({
    ...
    // Novas informações obrigatórias e estendidas da OS
...
export default function ServiceOrdersPage() {
    ...
    const onSubmit = async (data: OSFormValues) => {
        try {
            if (editingId) {
                // Parse checklist notes into JSONB format
                ...
                if (session?.user) {
                    ...
                    data.requester_id = session.user.id; // self request fallback
                }
    ...
    const filteredOrders = orders.filter(os =>
        ...
        os.vehicles?.plate?.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-white tracking-tight">Ordens de Serviço (OS)</h1>
                    <p className="text-slate-400 mt-1">Gerencie aberturas, acompanhamentos e encerramentos de manutenção.</p>
                </div>
            </div>
                    ...
                    <Table>
                        <TableHeader className="bg-black/40">
                            <TableRow className="border-white/5 hover:bg-transparent">
                                <TableHead className="text-slate-400">Ativo/Placa</TableHead>
                                ...
                                filteredOrders.map((os) => (
                                    <TableRow key={os.id} className="border-white/5 hover:bg-white/[0.02] hover:shadow-[inset_4px_0_0_0_#00E5FF] transition-all">
                                        <TableCell className="font-medium text-white">
                                            {os.vehicles?.plate || 'N/A'}
                                            <div className="text-xs text-slate-500">{os.vehicles?.model}</div>
                                        </TableCell>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Editar OS' : 'Nova Ordem de Serviço'}>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-300">Veículo (Ativo)</label>
                            ...
                            <div className="flex gap-2 mt-1">
                                ...
                                <Button
                                    ...
                                    onClick={async () => {
                                        setSuggesting(true);
                                        try {
                                            // Get active OS count per technician
                                            ...
                                            (activeOS || []).forEach(os => {
                                                if (os.technician_id) {
                                                    osCounts[os.technician_id] = (osCounts[os.technician_id] || 0) + 1;
                                                }
                                            });

                                            // Score: prefer least loaded tech (below max capacity)
                                            const scored = technicians
                                                .map(t => ({
                                                    ...t,
                                                    activeCount: osCounts[t.id] || 0,
                                                    available: (osCounts[t.id] || 0) < (t.max_active_os || 5)
                                                }))
