...
const planSchema = z.object({
    ...
    trigger_type: z.enum(['days']).default('days'), // Primarily time-based for facilities
    trigger_days: z.coerce.number().min(1, 'Dias é obrigatório e deve ser maior que zero'),
...
export default function FacilityPreventivePlansPage() {
    ...
    const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<PlanFormValues>({
        resolver: zodResolver(planSchema) as any,
        defaultValues: { trigger_type: 'days', active: true }
    });
    ...
    const fetchData = async () => {
        ...
        try {
            const { data: plansData, error: plansError } = await supabase
                ...
                .select(`
                    *,
                    assets_facilities ( name, location, category ),
                    maintenance_services ( name, sla_hours )
                `)
                .not('facility_id', 'is', null) // strict for this module
                .order('created_at', { ascending: false });

            if (plansError) throw plansError;
            setPlans(plansData || []);

            const { data: fData } = await supabase.from('assets_facilities').select('id, name, location').order('name');
    ...
    const handleDelete = async (id: string) => {
        if (!confirm('Excluir este plano predial?')) return;
    ...
    const openNewModal = () => {
        setEditingId(null);
        reset({ trigger_type: 'days', active: true });
        setIsModalOpen(true);
    };
    ...
    const filteredPlans = plans.filter(p =>
        ...
        p.assets_facilities?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        p.assets_facilities?.location?.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-white tracking-tight">Planos Preventivos Prediais</h1>
                    <p className="text-slate-400 mt-1">Gere rotinas de inspeção e manutenção (PMOC, limpeza de caixa d'água, etc) com base no tempo.</p>
                </div>
            </div>
                ...
                <CardHeader className="flex flex-row items-center justify-between border-b border-white/5 pb-4">
                    ...
                    <div className="flex items-center space-x-4">
                        <div className="relative">
                            ...
                            <Input
                                placeholder="Buscar plano ou local..."
                                value={searchTerm}
                    ...
                    <Table>
                        <TableHeader className="bg-black/40">
                            <TableRow className="border-white/5 hover:bg-transparent">
                                ...
                                <TableHead className="text-slate-400">Ativo Instalado</TableHead>
                                ...
                                filteredPlans.map((plan) => (
                                    <TableRow key={plan.id} className="border-white/5 hover:bg-white/[0.02] hover:shadow-[inset_4px_0_0_0_#00E5FF] transition-all">
                                        ...
                                        <TableCell className="text-slate-300">
                                            {plan.assets_facilities ? (
                                                <span>{plan.assets_facilities.name} <span className="text-slate-500 text-xs ml-1">({plan.assets_facilities.location})</span></span>
                                            ) : (
                                                <span className="text-slate-500 text-xs bg-slate-800/50 px-2 py-1 rounded">Múltiplos / Não definido</span>
                                            )}
                                        </TableCell>
                                        <TableCell>
                                            <span className="text-purple-400 font-mono text-sm">{plan.trigger_days} dias</span>
                                        </TableCell>
                                        <TableCell>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Editar Plano Preventivo' : 'Novo Plano Preventivo Predial'} size="lg">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">

                    <div>
                        <label className="block text-sm font-medium text-slate-300">Título do Plano</label>
                        <Input {...register('title')} placeholder="Ex: PMOC Mensal Ar Condicionado" className="mt-1 bg-black/20 border-white/10 text-white" />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-300">Instalação / Ativo Específico</label>
                            <select {...register('facility_id')} className="mt-1 block w-full px-3 py-2 bg-black/20 border border-white/10 rounded-md text-white focus:outline-none focus:ring-[#00E5FF] focus:border-[#00E5FF]">
                                <option value="" className="bg-[#0f0f14]">Selecione um ativo...</option>
                                {facilities.map(f => (
                                    <option key={f.id} value={f.id} className="bg-[#0f0f14]">{f.name} {f.location ? ` - ${f.location}` : ''}</option>
                                ))}
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="p-4 bg-black/30 border border-white/5 rounded-lg space-y-4 mt-4">
                        ...
                        <div className="flex items-center space-x-4">
                            <div className="flex-1">
                                <label className="block text-sm font-medium text-purple-400">Frequência de Execução (em Dias)</label>
                                <div className="mt-1 relative rounded-md shadow-sm">
                                    <Input type="number" {...register('trigger_days')} className="bg-black/20 border-white/10 text-white pr-16" placeholder="Ex: 30" />
                            ...
                            <div className="flex-1 pt-6 text-sm text-slate-400 leading-tight">
                                <p>Ex: <br /> 30 = Mensal <br /> 90 = Trimestral <br /> 180 = Semestral <br /> 365 = Anual</p>
                            </div>
                        </div>
                    </div>
