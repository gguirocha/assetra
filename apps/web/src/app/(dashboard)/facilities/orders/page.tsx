...
const osSchema = z.object({
    ...
    facility_id: z.string().min(1, 'Ativo é obrigatório'),
...
export default function FacilityOrdersPage() {
    ...
    const { register, handleSubmit, reset, setValue, formState: { errors, isSubmitting } } = useForm<OSFormValues>({
        ...
        defaultValues: { type: 'facility', priority: 'media', status: 'aberta' }
    });
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const { data: fData } = await supabase.from('assets_facilities').select('id, name, location, tag_qr_code');
    ...
    const openNewModal = () => {
        setEditingId(null);
        reset({ type: 'facility', priority: 'media', status: 'aberta' });
        setIsModalOpen(true);
    };
    ...
    const filteredOrders = orders.filter(os =>
        ...
        os.assets_facilities?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        os.assets_facilities?.location?.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-white tracking-tight">OS Predial (Facilities)</h1>
                    <p className="text-slate-400 mt-1">Gerencie a manutenção corretiva e preventiva da sua infraestrutura.</p>
                </div>
            </div>
                ...
                <CardHeader className="flex flex-row items-center justify-between border-b border-white/5 pb-4">
                    ...
                    <div className="flex items-center space-x-4">
                        <div className="relative">
                            ...
                            <Input
                                placeholder="Buscar por ativo, local ou descriç..."
                                value={searchTerm}
                    ...
                    <Table>
                        <TableHeader className="bg-black/40">
                            <TableRow className="border-white/5 hover:bg-transparent">
                                <TableHead className="text-slate-400">Ativo/Instalação</TableHead>
                                <TableHead className="text-slate-400">Descrição do Problema</TableHead>
                                ...
                                filteredOrders.map((os) => (
                                    <TableRow key={os.id} className="border-white/5 hover:bg-white/[0.02] hover:shadow-[inset_4px_0_0_0_#00E5FF] transition-all">
                                        <TableCell className="font-medium text-white">
                                            {os.assets_facilities?.name || 'N/A'}
                                            <div className="text-xs text-[#00E5FF]">{os.assets_facilities?.location}</div>
                                            {os.assets_facilities?.tag_qr_code && <div className="text-xs text-slate-500">Tag: {os.assets_facilities.tag_qr_code}</div>}
                                        </TableCell>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Editar OS Predial' : 'Nova Ordem de Serviço Predial'} size="lg">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">

                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-300">Instalação (Ativo)</label>
                            ...
                            {errors.facility_id && <p className="text-red-400 text-xs mt-1">{errors.facility_id.message}</p>}
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select {...register('status')} className="mt-1 block w-full px-3 py-2 bg-black/20 border border-white/10 rounded-md text-white focus:outline-none focus:ring-[#00E5FF] focus:border-[#00E5FF]">
                                ...
                                <option value="pecas" className="bg-[#0f0f14]">Aguardando Material</option>
                                <option value="concluida" className="bg-[#0f0f14]">Concluída</option>
                                <option value="cancelada" className="bg-[#0f0f14]">Cancelada</option>
                            </select>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-300">Técnico / Equipe Responsável</label>
                    ...
                    <div className="p-3 bg-black/30 border border-white/5 rounded-lg">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            ...
                            <div>
                                <label className="block text-xs font-medium text-slate-400">Materiais (R$)</label>
                    ...
                    <div className="flex justify-end space-x-3 pt-4 border-t border-white/10">
                        ...
                        <Button type="submit" disabled={isSubmitting} className="bg-gradient-to-r from-[#5B5CFF] to-[#00E5FF] hover:opacity-90 text-white border-0 glow-primary">
                            {isSubmitting ? 'Salvando...' : 'Salvar OS Predial'}
                        </Button>
                    </div>
                </form>
            </Modal>
